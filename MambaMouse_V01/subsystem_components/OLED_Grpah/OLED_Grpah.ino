...
//  }
  display.setChartCoordinates(0, 60);      //Chart lower left coordinates (X, Y)
  display.setChartWidthAndHeight(123, 55); //Chart width = 123 and height = 60
    display.setXIncrement(15);                //Distance between Y points will be 5px
    
  display.setYLimits(-100, 100);             //Ymin = 0 and Ymax = 100
  display.setYLimitLabels("-100", "100");    //Setting Y axis labels
  display.setYLabelsVisible(true);
  
  display.setAxisDivisionsInc(15, 50);    //Each 12 px a division will be painted in X axis and each 6px in Y axis
  display.setPlotMode(SINGLE_PLOT_MODE); //Set single plot mode
   display.setPointGeometry(POINT_GEOMETRY_NONE);
   
  display.drawChart(); //Update the buffer to draw the cartesian chart
  
  display.setLineThickness(6);
  display.updateChart(75);
  display.updateChart(75);
  display.updateChart(75);
  display.setLineThickness(7);
  display.updateChart(75);
  display.updateChart(75);
  display.updateChart(75);
  display.setLineThickness(8);
  display.updateChart(75);
  display.updateChart(75);
  display.updateChart(75);
