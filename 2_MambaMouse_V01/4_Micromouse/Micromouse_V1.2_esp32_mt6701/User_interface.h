...
const float R2 = 9.92;  // in kOhm
const float R1 = 10.11 + 10.05;
float voltage_level() {
   float input_volt = (map(analogRead(battMonitor_Pin), 0, 4095, 0, 3300)) / 1000.00 ;
   
   return (input_volt *(R1 + R2)/ R2 );
...
int enc_clickCount = 360 / 8;  // count needed to switch to a different mode (one rotation can click through 8 modes)
void encoderMode() {
